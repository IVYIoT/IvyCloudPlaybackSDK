...
public class TimeLineView extends View {
     ...
     * @param attrs   attrs
     */
    int timeScaleColor,timeTextColor,cloudVideoNorColor,middleLineColor,timeTextBackgroundColor;
    private void getAttrs(Context context, AttributeSet attrs) {
        ...
        timeTextBackgroundColor =  ta.getColor(R.styleable.TimeLineView_timeTextBgColor, 0);
        ta.recycle();
    }
    ...
    private void initPaint() {
        ...
        textBgPaint.setStyle(Paint.Style.FILL);//空心矩形框
        textBgPaint.setColor(timeTextBackgroundColor);
     ...
     * @param timeValue 初始值
     */
    public void initViewParam(long timeValue) {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        mWidth = getWidth();
        mNorLineDivider = mWidth / 6 / 20;//一屏幕120分钟 120个格子(60s/格)
    ...
    private void drawNorScalePart(Canvas canvas) {
        ...
        canvas.drawRect(new RectF(0, timeTextBgStart, mWidth, getHeight()), textBgPaint);
        for (int i = 0; drawCount <= mWidth / 2 + mProgressDis; i++) {
            ...
            if (xPosition + getPaddingRight() < mWidth) {
                ...
                if (div > 0) {
                    mTotalDiv += div;
                    canvas.drawRect(xPosition, videoStart, xPosition + div * mCurrentLineDivider, videoEnd, videoPaint);
                }
        ...
        canvas.drawLine(mWidth / 2, 0, mWidth / 2, videoEnd, whitePaint);

        float div = calculateInvisibleDiv();
        if (div > 0) {
            mTotalDiv += div;
            if (withProgress()) {
                canvas.drawRect(0, videoStart, div * mCurrentLineDivider, videoEnd, videoPaint);
            } else {
                canvas.drawRect(leftestPosition, videoStart, leftestPosition + div * mCurrentLineDivider, videoEnd, videoPaint);
            }
        }
        canvas.restore();
    }
                ...
                if (null != cloudListMap) {
                    for (Object o : cloudListMap.entrySet()) {
                        ...
                        if (key.substring(0, 4).equals(newResultKey)) {
                            ...
                            if (skip == 0)
                                skip = 1;
                            canvas.drawRect(xPosition, videoStart, xPosition + skip * mNorLineDivider / 6, videoEnd, videoPaint);
    ...
    /*竖屏的线间距*/
    public void setPortraitScreen(int screenHeight) {
        int timeLineHeight =  screenHeight *2 / 3;
        int timeTextHeight = screenHeight/3;
        longStart = timeLineHeight / 8;
        longEnd = timeLineHeight * 7/ 8;
        shortStart = timeLineHeight  / 4;
        shortEnd = timeLineHeight * 3/ 4;

        timeTextBgStart = timeLineHeight;
        textHeight = timeLineHeight + timeTextHeight*2/3;
        videoStart = 1;
        videoEnd = timeLineHeight-1;
    }
    ...
    public void setLandscapeScreen() {
        ...
        videoStart = 1 * mDensity;
        videoEnd = 55 * mDensity;
    }
